...
class Emoj extends Component {
    ...
    static propTypes = {
        emojCallBack: PropTypes.func
    }
    emojClick = (item) => {
        console.log(item)
        this.props.emojCallBack(item)
    }
    render () {
        const emojArr = [{
        ...
        const element = emojArr.map((item,index) => {
           return <li className="fl" onClick={() => this.emojClick(item)} style={styleLi} key={index}><img src={item.icon} title={item.value} alt={item.value}/></li>
        })
        return (
            <div>
                <ul className="clearfix" style={styleUl}>
                    {element}
                </ul>
            </div>
        )
